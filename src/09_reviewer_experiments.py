"""
9. Reviewer Revision Experiments — v3.0 (v4 TF-DFE Compatible)
================================================================
Changes from Code 9 v2.0 (which was already compatible with v3 / 7 fixes):

  CHANGE 1 compatibility:
    - audit_feature_counts() now also reports the three new diversity-pool
      members (MLP_BioTDA, kNN_SML, ElasticNet_Bio) added in Code 8 v4.
    - A new step run_diversity_analysis() reads pairwise_q_statistic.csv and
      pairwise_disagreement.csv produced by Code 8's compute_pairwise_diversity()
      and renders a summary table + heatmap for the paper.  This step is
      non-blocking: it skips gracefully if the CSVs are not yet present.

  CHANGE 2 compatibility:
    - audit_feature_counts() expected TDA feature count updated from 6
      (old: 3 stats × 2 homology dims) to the new ~20-feature count produced
      by the expanded TopologicalFeatureExtractor in Code 8 v4.
      The expected value is derived from 10 features × 2 dims = 20.
    - run_feature_ablation() TDA slice size comment updated accordingly.

  CHANGE 3 compatibility:
    - save_best_params_table() now records selective_abstain_threshold and
      ambiguous_margin / ambiguous_sample_weight from RevisionConfig.

  STEP8_SCRIPT_PATH updated to point to the v4 fixed script name so that
  build_full_feature_matrix() imports the correct preprocessor automatically.
  Update it back if you renamed the file differently.

  All other functions are unchanged from v2.0.
"""
...
class RevisionConfig:
    ...
    # FIX 3 compatibility: expected FCGR size after PCA compression.
    FCGR_PCA_COMPONENTS = 20

    # -----------------------------------------------------------------------
    # CHANGE 2 compatibility: expected TDA feature count after expansion.
    # Code 8 v4 produces 10 features × 2 homology dims = 20 TDA features.
    # (was 3 × 2 = 6 in v3)
    # -----------------------------------------------------------------------
    TDA_N_FEATURES_EXPECTED = 20   # CHANGE 2

    # -----------------------------------------------------------------------
    # CHANGE 3 compatibility: selective prediction parameters (for table)
    # -----------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CHANGE 2 compatibility: build_full_feature_matrix
# ---------------------------------------------------------------------------

def build_full_feature_matrix(variant_df):
    """Load the v4 preprocessor from Code 8 and build the full feature matrix.

    The v4 preprocessor (Code 8 v4) applies:
      - PCA(n_components=20) to the FCGR block  (FIX 3, retained)
      - TDA over FCGR/sequence space with ~20 features per sample  (CHANGE 2)

    So n_fcgr = 20, n_tda ≈ 20 here (vs n_tda = 6 in v3).
    All downstream slicing in run_feature_ablation and audit_feature_counts
    reflects these updated counts.
    """
    ...
    if hasattr(preprocessor, "tda_extractor"):
        ...
        def _patched_extract_global_tda(X, n_jobs=-1):
            ...
            print(f"Finding k-NN neighbors (k={n_neighbors}) for {n_samples} "
    ...
    n_fcgr     = len(preprocessor.fcgr_feature_names)   # FIX 3: PCA-compressed (20)
    n_tda      = len(preprocessor.tda_feature_names)    # CHANGE 2: expanded (~20)

    print(f"\nbuild_full_feature_matrix: n_standard={n_standard}, "
          f"n_fcgr={n_fcgr} (PCA-compressed), n_tda={n_tda} (CHANGE 2: over FCGR space)")
...
def build_tabular_features(variant_df, target_col, leakage_cols, fitted=None):
    ...
    if fitted is None:
        ...
        X_num   = pd.DataFrame(
            ...
            columns=num_cols, index=X_tab.index,
        )
        encoders = {}
    ...
    X_num = pd.DataFrame(
        ...
        columns=fitted["num_cols"], index=X_tab.index,
    )
...
def build_comparison_models(random_state):
    ...
    models["ExtraTrees"] = ExtraTreesClassifier(
        ...
        n_jobs=RevisionConfig.N_JOBS, random_state=random_state,
    )
    ...
    if XGBOOST_AVAILABLE:
        models["XGBoost"] = XGBClassifier(
            ...
            n_jobs=RevisionConfig.N_JOBS, verbosity=0,
            random_state=random_state, eval_metric="logloss",
        )
    if LGBM_AVAILABLE:
        models["LightGBM"] = LGBMClassifier(
            n_estimators=300, num_leaves=63, learning_rate=0.05,
            feature_fraction=0.8, n_jobs=RevisionConfig.N_JOBS,
...
def run_baseline_comparison(X_train, y_train, X_test, y_test, output_dir):
    ...
    ordered = [
        "model", "accuracy", "mcc", "precision", "recall", "specificity",
        "f1", "kappa", "auroc", "auprc", "brier", "fpr", "fnr", "npv",
        "tn", "fp", "fn", "tp",
    ]
...
def plot_combined_roc_pr(curve_data, output_dir, filename="Fig_All_Models_ROC_PR"):
    fig, (ax1, ax2) = plt.subplots(
        1, 2,
        figsize=(RevisionConfig.DOUBLE_COL, RevisionConfig.DOUBLE_COL / 2.4)
    )
    ...
    for i, (name, (y_true, proba)) in enumerate(curve_data.items()):
        ...
        ax2.plot(recall, precision, linewidth=1.3, color=palette[i],
...
def run_mcnemar_matrix(fitted_models, X_test, y_test, reference_name, output_dir):
    ...
    for name, pred in preds.items():
        ...
        rows.append({
            "reference": reference_name, "compared_model": name,
            "mcnemar_statistic": stat, "p_value": pval, "significance": sig,
        })
    df = pd.DataFrame(rows)
...
# ---------------------------------------------------------------------------
# CHANGE 1 compatibility: run_diversity_analysis
# ---------------------------------------------------------------------------

def run_diversity_analysis(output_dir):
    """Read Code 8 v4's pairwise diversity CSVs and produce a summary report.

    CHANGE 1 (Supervisor Request 1): Code 8 v4 now writes
      - pairwise_q_statistic.csv
      - pairwise_disagreement.csv
    produced by compute_pairwise_diversity() after training.
    ...
    This function:
      ...
      4. Prints a pass/fail verdict against the thresholds recommended by
         the supervisor (mean Q < 0.50 and mean disagreement > 0.05).

    Returns a summary dict (or None if the files are not present).
    """
    code8_out = Path(STAGE08_OUT)
    q_path    = code8_out / "pairwise_q_statistic.csv"
    dis_path  = code8_out / "pairwise_disagreement.csv"
    ...
    print("\n  [CHANGE 1 / Diversity Analysis — Code 9 Summary]")
    print(f"    Pool members: {names}")
    print(f"    Mean Q-statistic:    {mean_q:.4f}  "
          f"({'PASS' if q_target_pass else 'FAIL'} — target < 0.50)")
    print(f"    Max Q-statistic:     {max_q:.4f}")
    print(f"    Mean Disagreement:   {mean_dis:.4f}  "
    ...
    sns.heatmap(
        q_df, ax=axes[0], cmap="RdYlGn_r", vmin=-1, vmax=1,
    ...
    sns.heatmap(
        dis_df, ax=axes[1], cmap="Blues", vmin=0, vmax=0.30,
        annot=True, fmt=".2f", linewidths=0.2, annot_kws={"size": 4},
    )
    axes[1].set_title(
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "Fig_Diversity_Summary.png",
                dpi=RevisionConfig.DPI, bbox_inches="tight")
    plt.savefig(output_dir / "Fig_Diversity_Summary.tiff",
                dpi=RevisionConfig.DPI, bbox_inches="tight")
    plt.close()
...
# ---------------------------------------------------------------------------
# CHANGE 2 + FIX 3 compatibility: run_feature_ablation
# ---------------------------------------------------------------------------
...
def run_feature_ablation(
    ...
    CHANGE 2 compatibility:
    n_tda is now ~20 (was 6).  The TDA slice is updated accordingly.
    Comment text and the FCGR-compressed branch remain the same as v2.

    FIX 3 compatibility (retained from v2):
    When n_fcgr <= FCGR_PCA_COMPONENTS (default 20), the FCGR block is already
    PCA-compressed; skip the redundant inner-PCA loop.
    """
    ...
    def fit_eval(Xtr, Xte, tag):
        model = HistGradientBoostingClassifier(
            ...
            random_state=RevisionConfig.RANDOM_STATE, early_stopping=False,
        )
    ...
    if n_tda > 0:
        # CHANGE 2: TDA slice is now ~20 features (over FCGR space)
    ...
    # -----------------------------------------------------------------------
    # FIX 3 / CHANGE 2: If FCGR is already compressed, test sub-compression
    # -----------------------------------------------------------------------
    ...
    if n_fcgr > 0 and not already_compressed:
        ...
        print(f"  [FIX 3 / CHANGE 2] FCGR already PCA-compressed ({n_fcgr}D). "
    ...
    front = ["configuration", "n_features", "mcc", "accuracy", "f1",
             "auroc", "auprc", "fpr", "fnr"]
    df = df[
...
def run_imbalanced_evaluation(bio_model, n_standard, train_fitted, output_dir):
    ...
    X_imb, y_imb, _ = build_tabular_features(
        imb_test, RevisionConfig.TARGET_COL, RevisionConfig.LEAKAGE_COLS,
        fitted=train_fitted,
    )
    del imb_test
    ...
    df = pd.DataFrame(rows)
    front = ["threshold", "benign_pathogenic_ratio", "n_test", "fpr",
             "recall", "specificity", "precision", "mcc", "auroc", "auprc"]
    df = df[
        [c for c in front if c in df.columns]
        + [c for c in df.columns if c not in front]
    ]
...
# ---------------------------------------------------------------------------
# CHANGE 2 + FIX 3 + CHANGE 3 compatibility: audit_feature_counts
# ---------------------------------------------------------------------------

def audit_feature_counts(n_standard, n_fcgr, n_tda, output_dir):
    """Audit feature counts.

    CHANGE 2: expected TDA count is now TDA_N_FEATURES_EXPECTED (20),
    reflecting the expanded TopologicalFeatureExtractor in Code 8 v4 that
    computes 10 descriptors × 2 homology dimensions over the FCGR space.
    (was 3 × 2 = 6 in v3)

    FIX 3 (retained): expected FCGR count is FCGR_PCA_COMPONENTS (20).
    """
    expected_fcgr = RevisionConfig.FCGR_PCA_COMPONENTS          # FIX 3
    expected_tda  = RevisionConfig.TDA_N_FEATURES_EXPECTED       # CHANGE 2: 20 (was 6)
    rows = [
        {
...
def run_overfitting_analysis(X, y, n_standard, output_dir):
    ...
    for ts in train_sizes:
        X_tr, X_te, y_tr, y_te = train_test_split(
            X[:, :n_standard], y, train_size=ts, stratify=y,
        ...
        rows.append({
            "train_fraction": ts, "n_train": len(y_tr),
            "train_mcc":    matthews_corrcoef(y_tr, train_pred),
            "test_mcc":     matthews_corrcoef(y_te, test_pred),
            "train_test_gap": matthews_corrcoef(y_tr, train_pred) - matthews_corrcoef(y_te, test_pred),
    ...
    ax.fill_between(df["n_train"], df["test_mcc"], df["train_mcc"],
                    alpha=0.15, color="gray")
    ax.set_xlabel("Number of training samples"); ax.set_ylabel("MCC")
    ...
    plt.savefig(output_dir / "Fig_Overfitting_Check.png",
                dpi=RevisionConfig.DPI, bbox_inches="tight")
    plt.savefig(output_dir / "Fig_Overfitting_Check.tiff",
                dpi=RevisionConfig.DPI, bbox_inches="tight")
    plt.close()
    return df
...
def run_parameter_sensitivity(
    ...
    pivot = df.pivot(index="knora_k", columns="min_competence",
    ...
    ax.set_xlabel("min_competence"); ax.set_ylabel("knora_k")
    ax.set_title("Confident neighborhood fraction", fontweight="bold", loc="left")
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    plt.savefig(output_dir / "Fig_Parameter_Sensitivity.png",
                dpi=RevisionConfig.DPI, bbox_inches="tight")
    plt.close()
    return df
...
# ---------------------------------------------------------------------------
# Best-params table (updated for all v4 config keys — CHANGE 1, 2, 3)
# ---------------------------------------------------------------------------

def save_best_params_table(output_dir):
    rows = [
        # Core KNORA / neighbourhood parameters
        {"hyperparameter": "knora_k",              "value": RevisionConfig.KNORA_K,
         "change": "FIX 3"},
        {"hyperparameter": "min_competence",        "value": RevisionConfig.MIN_COMPETENCE_THRESHOLD,
         "change": ""},
        {"hyperparameter": "tda_neighbors",         "value": RevisionConfig.TDA_N_NEIGHBORS,
         "change": ""},
        {"hyperparameter": "sequence_window",       "value": RevisionConfig.SEQUENCE_WINDOW,
         "change": ""},
        {"hyperparameter": "fcgr_k_values",         "value": str(RevisionConfig.FCGR_K_VALUES),
         "change": ""},
        {"hyperparameter": "tda_homology_dims",     "value": str(RevisionConfig.TDA_HOMOLOGY_DIMS),
         "change": ""},
        {"hyperparameter": "test_size",             "value": RevisionConfig.TEST_SIZE,
         "change": ""},
        {"hyperparameter": "random_state",          "value": RevisionConfig.RANDOM_STATE,
         "change": ""},
        # Feature sizes
        {"hyperparameter": "n_fcgr_features_raw",
         "value": sum(4**k for k in RevisionConfig.FCGR_K_VALUES),
         "change": "FIX 3"},
        {"hyperparameter": "n_fcgr_features_pca",  "value": RevisionConfig.FCGR_PCA_COMPONENTS,
         "change": "FIX 3"},
        {"hyperparameter": "n_tda_features",        "value": RevisionConfig.TDA_N_FEATURES_EXPECTED,
         "change": "CHANGE 2 (was 6)"},
        # FIX 2
        {"hyperparameter": "weak_learner_margin",   "value": 0.10,
         "change": "FIX 2"},
        # FIX 5
        {"hyperparameter": "threshold_search_low",  "value": 0.45,
         "change": "FIX 5"},
        {"hyperparameter": "threshold_search_high", "value": 0.65,
         "change": "FIX 5"},
        # CHANGE 1: diversity pool
        {"hyperparameter": "diversity_pool_additions",
         "value": "MLP_BioTDA, kNN_SML, ElasticNet_Bio",
         "change": "CHANGE 1"},
        # CHANGE 3: label ceiling parameters
...
# ---------------------------------------------------------------------------
# main()  — v3.0 (CHANGE 1 diversity step added)
# ---------------------------------------------------------------------------
...
def main():
    ...
    if full is not None:
        ...
        n_fcgr     = full["n_fcgr"]   # FIX 3: PCA-compressed (20)
        n_tda      = full["n_tda"]    # CHANGE 2: ~20 (over FCGR space)
        tab_fitted = None
        print(
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=RevisionConfig.TEST_SIZE,
        stratify=y, random_state=RevisionConfig.RANDOM_STATE,
    )
    ...
    # ------------------------------------------------------------------
    # CHANGE 1: Diversity analysis — read Code 8 v4 output CSVs
    # ------------------------------------------------------------------
    print("\nStep: Diversity Analysis (CHANGE 1)")
    t0 = time.time()
    ...
    if "XGBoost" in fitted_models:
        run_lime_analysis(fitted_models["XGBoost"], X_train_bio, X_test_bio, y_test, out, feature_names=tab_fitted["feature_names"])
